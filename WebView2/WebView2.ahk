...
class WebView2 {
	...
	/**
	 * Converts a JSON string into native AHK v2 types using an HTMLfile COM object.
	 * @param {String} jsonStr 
	 * @returns {Any} Native AHK Map, Array, or primitive value.
	 */
	static JSONParse(jsonStr) {
		...
		try return Convert(js.JSON.parse(jsonStr))
		catch
			return jsonStr
		...
		Convert(obj) {
			...
			if js.Array.isArray(obj) {
				arr := []
				loop obj.length
					arr.Push(Convert(obj.%A_Index - 1%))
				return arr
			}
			m := Map()
	...
	static AsyncHandler(&p, wrapper := 0) {
		...
		static handler(this, err, result := '') {
			try {
				...
				; Ignore stale WebView2 callbacks.
			}
		}
	}
	...
	class Core extends WebView2.Base {
		...
		Eval(js) {
				wrapper := "(() => { try { let r = (" js "); return JSON.stringify(r === undefined ? null : r); } catch(e) { return JSON.stringify({__error: e.message}); } })()"
				rawJson := this.ExecuteScriptAsync(wrapper)
				if (rawJson = "" || rawJson = "null")
					return ""
				
				parsed := WebView2.JSONParse(rawJson)
				if (IsObject(parsed) && parsed is Map && parsed.Has("__error"))
					throw Error("JS Scraper Error: " . parsed["__error"])
				
				return parsed
			}

			/**
			 * Waits until the document readyState is 'interactive' or 'complete'.
			 * @param {Integer} timeoutMs - Max wait time in milliseconds (default: 15s).
			 */
			WaitLoad(timeoutMs := 15000) {
				start := A_TickCount
				while (A_TickCount - start < timeoutMs) {
					state := this.Eval("document.readyState")
					if (state = "complete" || state = "interactive")
						return true
					Sleep(100)
				}
				throw TimeoutError("Page load timed out.", -1)
			}

			/**
			 * Waits until an element matching the CSS selector exists in the DOM.
			 * @param {String} selector - CSS Selector.
			 * @param {Integer} timeoutMs - Max wait time in milliseconds (default: 10s).
			 * @returns {Boolean}
			 */
			WaitElement(selector, timeoutMs := 10000) {
				start := A_TickCount
				esc := StrReplace(selector, "'", "\'")
				while (A_TickCount - start < timeoutMs) {
					if this.Eval("!!document.querySelector('" esc "')")
						return true
					Sleep(100)
				}
				return false
			...
			/**
			 * Retrieves innerText of the target element.
			 * @param {String} selector - CSS Selector.
			 */
			GetText(selector) {
				esc := StrReplace(selector, "'", "\'")
				return this.Eval("document.querySelector('" esc "')?.innerText ?? ''")
			}

			/**
			 * Retrieves innerHTML or outerHTML of the target element.
			 * @param {String} selector - CSS Selector.
			 * @param {Boolean} outer - Return outerHTML if true, else innerHTML.
			 */
			GetHTML(selector, outer := false) {
				esc := StrReplace(selector, "'", "\'")
				prop := outer ? "outerHTML" : "innerHTML"
				return this.Eval("document.querySelector('" esc "')?. " prop " ?? ''")
			}

			/**
			 * Gets an attribute value from an element.
			 * @param {String} selector - CSS Selector.
			 * @param {String} attr - Attribute name (e.g. 'href', 'src', 'value').
			 */
			GetAttribute(selector, attr) {
				escSel := StrReplace(selector, "'", "\'")
				escAttr := StrReplace(attr, "'", "\'")
				return this.Eval("document.querySelector('" escSel "')?.getAttribute('" escAttr "') ?? ''")
			}

			/**
			 * Scrapes multiple elements into an AHK Array.
			 * @param {String} selector - CSS selector matching multiple nodes.
			 * @param {String|Array} props - A single property/attribute name or an Array of names.
			 * @returns {Array} Array of string values or Array of Maps.
			 */
			ScrapeAll(selector, props := "innerText") {
				escSel := StrReplace(selector, "'", "\'")
				
				if (props is String) {
					escProp := StrReplace(props, "'", "\'")
					js := "Array.from(document.querySelectorAll('" escSel "')).map(e => e['" escProp "'] || e.getAttribute('" escProp "') || '')"
					return this.Eval(js)
				} else if (props is Array) {
					js := "Array.from(document.querySelectorAll('" escSel "')).map(e => {"
					js .= "let o = {};"
					for p in props {
						ep := StrReplace(p, "'", "\'")
						js .= "o['" ep "'] = e['" ep "'] || e.getAttribute('" ep "') || '';"
					}
					js .= "return o; })"
					return this.Eval(js)
				}
			...
			/**
			 * Triggers a click on the target element.
			 * @param {String} selector - CSS Selector.
			 */
			ClickElement(selector) {
				esc := StrReplace(selector, "'", "\'")
				return this.Eval("(() => { let e = document.querySelector('" esc "'); if(e) { e.click(); return true; } return false; })()")
			}

			/**
			 * Inputs text into an input element and dispatches 'input' and 'change' events.
			 * @param {String} selector - CSS Selector.
			 * @param {String} value - Text to input.
			 */
			SetInputValue(selector, value) {
				escSel := StrReplace(selector, "'", "\'")
				escVal := StrReplace(StrReplace(value, "\", "\\"), "'", "\'")
				js := "(() => { let e = document.querySelector('" escSel "'); if(e) { e.value = '" escVal "'; e.dispatchEvent(new Event('input', {bubbles:true})); e.dispatchEvent(new Event('change', {bubbles:true})); return true; } return false; })()"
				return this.Eval(js)
			}
		}
